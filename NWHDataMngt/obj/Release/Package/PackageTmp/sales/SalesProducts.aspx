  ...
  <link rel="stylesheet" type="text/css" href="../style/themes/base/jquery-ui.css?v=1" />
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			jtExpPrgIDE = document.getElementById('txtExpPrgIDE');
		...
		function GetProductCodesManaged(ptype, pcode, reg, thick, spec, grade, sort, act) {
			var url = "../shared/AJAXServices.asmx/SelectProductCodesManaged";
			var MyData = "{'PgID':'" + jiPageID.toString() + "','ObjID':'0','ProdType':'" + ptype + "','ProdCode':'" + pcode + "','Reg':'" + reg + "','Thickness':'" + thick + "','Specie':'" + spec + "',";
			MyData = MyData + "'Grade':'" + grade + "','Sort':'" + sort.toString() + "','Active':'" + act.toString() + "','ByID':'" + jiByID.toString() + "'}";
			//alert(MyData);
			getJSONReturnDataAs(url, MyData, function (response)
			{ MyProductCodeList = response; });
			return false;
		}	
		...
		function UpdateProductData(id, reg, pcode, desc, prc, cmt, spec, thick, grade, len, clr, sort, mill, noprint, nattach, mngd, trkd, pt, expp) {
			var url = "../shared/GridSupportServices.asmx/UpdateSalesProductData";
			var MyData = "{'PgID':'" + jiPageID.toString() + "','ObjID':'0','ID':'" + id.toString() + "','Region':'" + reg + "','Prod':'" + pcode + "','Desc':'" + desc + "','Price':'" + prc + "',";
			MyData = MyData + "'Comments':'" + cmt + "','Specie':'" + spec + "','Thickness':'" + thick + "','Grade':'" + grade + "','Len':'" + len + "','Color':'" + clr + "','Sort':'" + sort + "','Milling':'" + mill + "',";
			MyData = MyData + "'NoPrint':'" + noprint + "','ProdType':'" + pt + "','ExpPrgID':'" + expp + "','NbrAttach':'" + nattach.toString() + "','Mngd':'" + mngd.toString() + "','Track':'" + trkd.toString() + "',";
		...
		function PopulateGradeList() {
			...
			if (!IsContentsNullUndefinedGu(MyCodeListData)) {
				...
				fillDropDownListGu(MyCodeListData, jselGradeF, 0, 'CodeDescCompAbbreviated', 'CatCode');
				// edit list
				ClearDDLOptionsGu('selItemGradeE', 1);
				fillDropDownListGu(MyCodeListData, jselItemGradeE, 0, 'CodeDescCompAbbreviated', 'CatCode');
			}
			return false;
		}
		...
		function PopulateProductGrid() {
			...
			var cnames = ['ProductManagedID', 'sIsActive', 'sIsManaged', 'Specie', 'Thickness', 'Grade', 'Region', 'ProductCode', 'ProdDesc', 'Length', 'Color', 'Sort', 'Milling', 'NoPrint', 'sPrice', 'Comments', 'ACTION', '', '', '', '', '', '', '', '', '', '', ''];
		...
		function PopulateProductList() {
			...
			GetProductCodesManaged('', '', '', '', '', '', 0, 1);
			//alert(MyProductListMini.length);
			if (!IsContentsNullUndefinedGu(MyProductCodeList)) {
				ClearDDLOptionsGu('selProductCodeE', 1);
				fillDropDownListGu(MyProductCodeList, jselProductCodeE, 0, 'ProdDesc', 'ProductCode');
			}
			return false;
		}
		...
		function ChangeCheckBox(id, row, col, objid, chkd) {
			//alert('Fired -' + id + '/' + objid + '/' + chkd);
			if (id > 0) {
				switch (objid) {
					case 1:
						var ischecked = '0';
		...
		function EditOneRec(id, row, objid) {
			...
			switch (objid) {
				case 1: // prod
					alert(MyProductData[row]['ProductCode'].toString() + '/' + MyProductData[row]['Product'].toString() + '/' + MyProductData[row]['ProdDesc'].toString());
					...
					jtaProdDescE.value = MyProductData[row]['ProdDesc'].toString();
					jselRegionE.value = MyProductData[row]['Region'].toString();
					jtPriceE.value = MyProductData[row]['sPrice'].toString();
					...
					if (MyProductData[row]['sIsManaged'].toString() === 'Yes') {
						jchkManagedOnly.checked = true;
					}
					else {
						jchkManagedOnly.checked = false;
					}
					jtExpPrgIDE.value = MyProductData[row]['ExpPrgID'].toString();
		...
		function SaveProdItemData() {
			...
			prc = prc.replace(',', '');
			var pt = jselProductTypeE.value; // 'HD';
			...
			UpdateProductData(id, reg, pcode, desc, prc, cmt, spec, thick, grade, len, clr, sort, mill, noprint, nattach, mngd, trkd, pt, expp);
			CloseProdItemEdit();
			DataCall1();
			return false;
		}
        ...
        <tr>
          ...
          <td style="vertical-align:middle;">
			...
			<div id="divPAGEHEADER" style="width:98%;margin-left:10px;margin-bottom:10px;">
				...
				<tr>
					<td style="width:25%;">
						<span id="spnREgPriceGVLbllit" style="color:blue;font-size:14pt;font-weight:bold;"><asp:Literal ID="RegPriceGVLbllit" runat="server" Text="Sales Product Management"></asp:Literal></span>
					</td>
					<td style="width:25%;">
					...
					<table style="padding:4px;margin:auto auto;"><tr><td>
						...
						<tr>
							...
							<td style="text-align:left;" colspan="5">
								<input type="text" id="txtCommentPmE" style="width:500px;" />
							</td>
							<td style="text-align: right; vertical-align: top;">
								Exp&nbsp;Prg&nbsp;ID:&nbsp;
							</td>
							<td style="text-align: left;">
								<input type="text" id="txtExpPrgIDE" style="text-align:right;width:80px;" value="0.00" />&nbsp;
							</td>
						</tr>
						</table>
					</td></tr></table>
