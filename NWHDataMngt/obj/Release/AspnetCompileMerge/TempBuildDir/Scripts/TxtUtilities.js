...
Contents:
  ...
  PrepareHTMLForViewTx(val, typ)
	PrepareJSONForSpecialHTMLTx(tval, typ)
  PrepareHTMLForSpecialViewTx(val, typ)
  getChrTx(iChr)
  StringifyTx(txt)
  ...
  compareTwoDocumentsWithResultsTx(filenm1, filenm2, tlist, objid)
	...
	ValidateEmailStringTx(txt)
	ValidateNumericStringTx(txt)
	ValidateStringFieldTx(objid, typ, reqd, minchar, maxchar, badchars)
*/
var jsTUfVersion = '1.1.9';
var jsTUfVersDate = '11/8/2017';
...
function EncodeSingleQuotesTx(val) {
	...
	val = val.replace(/\&/gi, '[XAMPCHAR]');
	return val;
}

function UncodeSingleQuotesTx(val) {
	if (val === undefined || val === null) { val = ''; }
	val = val.replace(/[XSQUOTEGRP]/gi, "''");
	val = val.replace(/[XSQUOTE]/gi, "'");
	val = val.replace(/[XDQUOTE]/gi, '"');
	val = val.replace(/[CMDTERMINATE]/gi, ';');
	val = val.replace(/[XAMPCHAR]/gi, '&');
	return val;
}
...
function PrepareJSONForSpecialHTMLTx(tval, typ) {
	if (tval === undefined || tval === null) { tval = ''; }
	var val = tval.toString();
	val = val.replace(/\t/g, '  ');
	//val = val.replace(/\&/gi, '&amp;');
...
function PrepareHTMLForSpecialViewTx(tval, typ) {
	if (tval === undefined || tval === null) { tval = ''; }
	var val = tval.toString();
	//val = val.replace(/&amp;/gi, '&');
	val = val.replace(/&#39;/gi, "'");
...
function ValidateEmailStringTx(txt) {
	var okay = true;
	if (txt === undefined || txt === null) { txt = ''; }
	var specChars = "!#$^&%*()+=[]\/{}|:<>?,'";
	for (var i = 0; i < specChars.length; i++) {
		if (txt.indexOf(specChars[i]) > -1) {
			okay = false;
			break;
		}
	}
...
function ValidateNumericStringTx(txt) {
	var okay = true;
	if (txt === undefined || txt === null) { txt = ''; }
	var specChars = "!#$^&%*()+=[]\/{}|:<>?,'-";
	for (var i = 0; i < specChars.length; i++) {
	...
	for (var i = 0; i < TCode.length; i++) {
		...
		if (cc < 48 || cc > 57 && cc !== 46 && cc !== 44) {
			okay = false; //alert('Input is not alphanumeric');
		}
	}
	return okay;
}

function ValidateStringFieldTx(objid, typ, reqd, minchar, maxchar, badchars, reqchars) {
	//reqd: 0-not required, 1-required, 2-cannot be 0
	var okay = 0;
	var val = '';
	if (typ === 0) {
		...
		val = document.getElementById(objid).innerHTML;
	}
	if (val === '' && reqd > 0) {
		okay = 1;
	}
		...
		if (minchar > 0) {
			if (val.length < minchar) {okay = 2;}
		}
		if (maxchar > 0) {
			if (val.length > maxchar) { okay = 3; }
		}
	}
	if (okay === true && reqd === 2 && val === '0') {okay = 4;}
	...
	if (okay === 0 && reqchars.length > 0) {
		for (var i = 0; i < reqchars.length; i++) {
			if (val.indexOf(reqchars[i]) < 0) {
				okay = 6;
				break;
			}
		}
